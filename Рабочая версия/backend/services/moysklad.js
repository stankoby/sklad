...
class MoySkladService {
  ...
  async getAllProducts() {
    ...
    // Базовые товары
    await fetchAll('/entity/product');

    // Модификации (variants)
    await fetchAll('/entity/variant');

    return rows;
  }

  // Скачать изображение товара (product/variant) и вернуть бинарные данные + content-type.
  async getProductImageDownload(productId) {
    ...
    let images = await fetchImages(`/entity/product/${id}/images`);
    if (!images || images.length === 0) {
      images = await fetchImages(`/entity/variant/${id}/images`);
    }
    ...
    const image = images.find((img) => img?.meta?.downloadHref || img?.miniature?.href) || images[0];
    const downloadHref = image?.meta?.downloadHref || null;
    const miniatureHref = image?.miniature?.href || null;
    const targetUrl = downloadHref || miniatureHref;

    if (!targetUrl) {
      return null;
    }

    // Если downloadHref отсутствует, запрос на miniature.href инициирует генерацию миниатюры.
