  ...
  try {
    ...
    for (const it of available) {
      ...
      zonesMap.get(key).push({
        ...
        qty_to_collect: Math.max(0, (it.planned_qty ?? 0) - (it.scanned_qty ?? 0))
      });
    }

    const rackOrderList = Object.keys(rackOrder)
      .sort((a, b) => rackOrder[a] - rackOrder[b])
      .map(Number);
    const orderedKeys = rackOrderList.filter((rack) => zonesMap.has(rack));
    const extraKeys = [...zonesMap.keys()].filter((key) => !rackOrderList.includes(key));
    const zones = [...orderedKeys, ...extraKeys].map((rack) => ({
      rack,
    ...
    const totalToCollect = available.reduce((sum, i) => sum + Math.max(0, (i.planned_qty ?? 0) - (i.scanned_qty ?? 0)), 0);
