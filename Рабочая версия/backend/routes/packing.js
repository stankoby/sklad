...
router.get('/tasks/:id/route-sheet', async (req, res) => {
  try {
    ...
    const task = db.prepare('SELECT * FROM packing_tasks WHERE id = ?').get(taskId);
    if (!task) return res.status(404).json({ error: 'Task not found' } catch (e) { console.error(e); });
    ...
    const rackOrderList = [
      41, 42, 37, 38, 39, 40, 52, 51, 50, 49, 32, 36, 31, 35, 30, 34, 29, 33, 28, 23, 18, 19, 24,
      20, 25, 21, 26, 22, 27, 48, 47, 46, 12, 17, 11, 16, 10, 15, 9, 14, 8, 13, 1, 2, 3, 4, 5, 6, 7,
      45, 44, 43
    ];

    const rackIndex = (rack) => {
      const idx = rackOrderList.indexOf(Number(rack));
      return idx === -1 ? Number.POSITIVE_INFINITY : idx;
    };

    available.sort((a, b) => {
      const ar = rackIndex(a.rack);
      const br = rackIndex(b.rack);
    ...
    const orderedKeys = Array.from(zonesMap.keys()).sort((a, b) => {
      const ar = Number(a);
      const br = Number(b);
      const aIdx = rackOrderList.indexOf(ar);
      const bIdx = rackOrderList.indexOf(br);
      if (aIdx === -1 && bIdx === -1) return ar - br;
      if (aIdx === -1) return 1;
      if (bIdx === -1) return -1;
      return aIdx - bIdx;
    });

    const zones = orderedKeys.map((rack) => ({
      rack,
      rows: zonesMap.get(rack) ?? []
    }));
    ...
    const noStockItems = noStock.map((r) => ({
