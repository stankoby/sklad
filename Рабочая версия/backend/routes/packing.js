...
router.get('/tasks/:id/route-sheet', async (req, res) => {
  try {
    ...
    available.sort((a, b) => {
      ...
      const ar = a.rack ?? 9999;
      const br = b.rack ?? 9999;

      const as = a.shelf ?? 9999;
      const bs = b.shelf ?? 9999;

      const ac = (a.cell ?? 'ZZZ').toString();
      const bc = (b.cell ?? 'ZZZ').toString();

    const zonesMap = new Map(); // rack -> items
    for (const it of available) {
      const key = it.rack ?? 'Без ячейки';

    const zones = Array.from(zonesMap.entries()).map(([rack, rows]) => ({
